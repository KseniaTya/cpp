...
class grCpp2Types(object):
    ...
    @classmethod
    def get_const_from_str(cls, cpptype, const_value, ctx):
        # если const_value это строка
        if type(const_value) is str:
            # если тип cpptype - символ
            if cpptype == cls.char:
            ...
            # если тип cpptype - с плавающей точкой
            elif cpptype in [cls.float, cls.double]:
                return cpptype(float(const_value))

            # если тип cpptype - целочисленный
            elif cpptype in [cls.short, cls.int]:
                return cpptype(int(const_value))

            # если тип cpptype - массив и его эл-ты - символы
            elif isinstance(cpptype, ir.ArrayType) and cpptype.element == cls.char:
                ...
                # создаём новую константу типа cpptype с помощью ir.Constant,
                # передаём в нее массив байтов, представляющих строку str_val с кодировкой 'ascii'
                return ir.Constant(cpptype, bytearray(str_val, 'ascii'))

            else:
                raise SemanticError(msg=f"No known conversion: {const_value} to {cpptype}")
        else:
            raise SyntaxError(msg=f"get_const_from_str doesn't support const_value which is a {str(type(const_value))}",
                              ctx=ctx)
    ...
    def cast_type(cls, builder, target_type, value, ctx):
        ...
        elif type(value.type) == ir.ArrayType and type(
                target_type) == ir.PointerType and value.type.element == target_type.pointee:
        ...
        elif isinstance(value.type, ir.ArrayType) and isinstance(target_type,
                                                                 ir.ArrayType) and value.type.element == target_type.element:
